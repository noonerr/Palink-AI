(async () => {
  ...
  const devOk = await page.evaluate(async () => {
    const r = await fetch('/api/users/me/settings');
    if (!r.ok) return null;
    const d = await r.json();
    return d?.developer_mode_enabled === true;
  });

  if (!devOk) {
    console.log(JSON.stringify({error:'developer_mode_not_enabled'}));
    await ctx.close();
    return;
  }
  ...
  await page.evaluate(() => {
    ...
    window.fetch = async (...args) => {
      if (String(args[0]).includes('/api/chat')) {
        const long = Array.from({length:50}, (_,i)=>'AI line '+(i+1)+' '+ 'x'.repeat(60)).join('\n') + '\n\n```js\nconsole.log(Array.from({length:120}, (_,i)=>i).join("-"))\n```';
  ...
  await page.screenshot({ path:'D:/项目/Palink-AI/test-screenshots/mobile-code-overflow.png', fullPage:true });
