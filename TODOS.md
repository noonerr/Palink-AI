...
- **线上修复：重 roll/swipe 开场白输出元信息而非剧情（2026-08-28 施工）**: 现象=会话 b7b9a8e6 消息 2249 重 roll 开场白后模型输出「角色卡设定与扮演规划说明」类元信息。根因=regenerate/swipe 端点把目标开场白从 messages 移除后（include_user_message=False + 空历史），prompt 只剩 system 轮收尾，模型无"正在对话"锚点，弱模型（deepseek-v4-flash）把角色卡定义堆当待说明材料。ST script.js:4780 对同场景有 "hack for regeneration of the first message"（空历史补一条 user 轮），其 OAI 路径更以 user 角色注入 jailbreak 作锚。修复=character_ext.py 新增 `_ensure_conversation_anchor`（按卡片语言中/英双文案），regenerate/swipe 两端点在移除目标消息后调用；有 user/assistant 轮时零影响。新增 test_opening_regen_anchor.py 4 例；全量 **1084/0**；镜像重建 healthy 且 GUARD-IN-IMAGE
