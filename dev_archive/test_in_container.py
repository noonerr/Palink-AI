...
def extract_chara_card_from_png(png_data):
    ...
    # Check ALL text chunk types
    text_chunks = [c for c in chunks if c['type'] in ['tEXt', 'zTXt', 'iTXt']]
    print(f"\nFound {len(text_chunks)} text chunks (tEXt/zTXt/iTXt)")
    ...
    for chunk_idx, chunk in enumerate(text_chunks):
        ...
        if null_pos != -1:
            try:
                ...
                # Try even if keyword not in list!
                if keyword_lower not in supported_keywords:
                    print(f"  Keyword not in supported list, but trying anyway...")
