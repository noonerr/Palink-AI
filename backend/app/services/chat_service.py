...
class ChatService:
    ...
    def getMessages(self, session_id: str) -> List[ChatMessage]:
        """返回指定会话的全部消息，按创建时间升序排列。"""
        return (
    ...
    def getMessageById(self, message_id: int) -> Optional[ChatMessage]:
        """根据消息 ID 返回单条消息；不存在时返回 None。"""
        return (
            self.db.query(ChatMessage)
    ...
    def clearMessages(self, session_id: str) -> int:
        """删除指定会话的全部消息，返回删除的行数。"""
        deleted = (
            self.db.query(ChatMessage)
            .filter(ChatMessage.session_id == session_id)
    ...
    def getMessageTokenCount(self, message_id: int) -> int:
        """返回指定消息的 token 计数（tokens 字段）。

        不存在或字段为空时返回 0。注意：仅返回已持久化的 tokens 值，
        不会触发重新分词。
        """
        message = self.getMessageById(message_id)
        if not message:
            return 0
        return int(message.tokens or 0)
