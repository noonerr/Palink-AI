...
function getPalinkManagedApiUrl() {
    return `${location.origin}/api/openai/v1`;
}

function isPalinkManagedCustomUrl(url) {
    const normalized = String(url || '').trim().replace(/\/+$/, '');
    if (!normalized) {
        return true;
    }
    const managedUrl = getPalinkManagedApiUrl().replace(/\/+$/, '');
    return normalized === managedUrl;
}

function shouldApplyPalinkManagedDefaults() {
    if (!globalThis.__PALINK_ST_EMBEDDED) {
        return false;
    }
    if (!oai_settings) {
    ...
    const source = String(oai_settings.chat_completion_source || '').toLowerCase();
    if (source && source !== chat_completion_sources.CUSTOM) {
        return false;
    }
    return isPalinkManagedCustomUrl(oai_settings.custom_url);
}

async function getStatusOpen() {
    const palinkStatus = getPalinkManagedApiStatus();
    if (palinkStatus) {
        if (shouldApplyPalinkManagedDefaults()) {
            oai_settings.chat_completion_source = chat_completion_sources.CUSTOM;
            if (globalThis.__PALINK_ST_MODEL) {
                oai_settings.custom_model = globalThis.__PALINK_ST_MODEL;
                oai_settings.openai_model = globalThis.__PALINK_ST_MODEL;
            }
            oai_settings.custom_url = getPalinkManagedApiUrl();
        }
        try {
            ...
            if (response.ok) {
                ...
                if (responseData.model && shouldApplyPalinkManagedDefaults()) {
                    oai_settings.custom_model = responseData.model;
                    oai_settings.openai_model = responseData.model;
                }
                setOnlineStatus(responseData.status || palinkStatus);
            } else {
                setOnlineStatus(palinkStatus);
            }
        } catch (error) {
            console.warn('Palink managed API status check failed, keeping managed status.', error);
            setOnlineStatus(palinkStatus);
        }
...
async function onConnectButtonClick(e) {
    e.stopPropagation();
    const palinkStatus = getPalinkManagedApiStatus();
    if (palinkStatus && shouldApplyPalinkManagedDefaults()) {
        startStatusLoading();
        oai_settings.chat_completion_source = chat_completion_sources.CUSTOM;
        oai_settings.custom_url = getPalinkManagedApiUrl();
        if (globalThis.__PALINK_ST_MODEL) {
