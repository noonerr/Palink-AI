...
class OptimizedMemoryService:
    ...
    async def get_context_fast(self, user_id: int, session_id: str, query: str) -> Dict:
        ...
        try:
            ...
            # 计算延迟
            context["latency_ms"] = (datetime.utcnow() - start_time).total_seconds() * 1000
    ...
    def _store_message(self, user_id: int, session_id: str, 
        ...
        try:
            ...
            sql = text("""
                ...
                VALUES 
                (:user_id, :session_id, 'user', :user_msg, 0.5, NOW()),
                (:user_id, :session_id, 'assistant', :assistant_msg, 0.5, NOW())
            """)
            self.db.execute(sql, {
                "user_id": user_id,
    ...
    async def _extract_key_facts(self, user_id: int, user_msg: str, assistant_msg: str):
        ...
        if new_facts:
            ...
            self._set_cache(facts_key, updated_facts, ttl=3600)  # 1小时缓存
    
    # ========== 缓存工具 ==========
    
    @classmethod
    def _get_cache(cls, key: str) -> Any:
        """获取缓存"""
        if key in cls._cache:
            expire_time = cls._cache_ttl.get(key, 0)
    ...
    @classmethod
    def _set_cache(cls, key: str, value: Any, ttl: int = 300):
        """设置缓存"""
        cls._cache[key] = value
        cls._cache_ttl[key] = datetime.utcnow().timestamp() + ttl
