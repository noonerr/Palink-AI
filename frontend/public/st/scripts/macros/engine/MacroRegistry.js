...
function detectMacroSource() {
    ...
    const isExtension = stack.some(line => line.includes('/st/scripts/extensions/'));
    const isThirdParty = stack.some(line => line.includes('/st/scripts/extensions/third-party/'));

    let source = 'unknown';
    if (isThirdParty) {
        const match = stack.find(line => line.includes('/st/scripts/extensions/third-party/'));
        if (match) {
            source = match.replace(/^.*?\/scripts\/extensions\/third-party\/([^/]+)\/.*$/, '$1');
        }
    } else if (isExtension) {
        const match = stack.find(line => line.includes('/st/scripts/extensions/'));
        if (match) {
