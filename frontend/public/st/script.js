...
globalThis.SillyTavern.selectCharacterById = selectCharacterById;
globalThis.SillyTavern.printMessages = printMessages;
globalThis.SillyTavern.setCharacterId = setCharacterId;
globalThis.SillyTavern.clearChat = clearChat;
globalThis.SillyTavern.setOnlineStatus = setOnlineStatus;
globalThis.SillyTavern.displayOnlineStatus = displayOnlineStatus;
globalThis.SillyTavern.isAppReady = false;
...
async function firstLoadInit() {
    ...
    splashLogo.src = '/st/img/logo.png';
    ...
    await fixViewport();
    globalThis.SillyTavern.isAppReady = true;
...
function getPalinkManagedApiStatus() {
    if (!globalThis.__PALINK_ST_EMBEDDED) {
        return null;
    }
    const model = globalThis.__PALINK_ST_MODEL || globalThis.__PALINK_ST_BOOT_CONTEXT?.model || '';
    return model ? `Palink API: ${model}` : 'Palink API';
}
...
export function cancelStatusCheck(reason = 'Manually cancelled status check') {
    ...
    setOnlineStatus(getPalinkManagedApiStatus() || 'no_connection');
}

export function displayOnlineStatus() {
    const palinkStatus = getPalinkManagedApiStatus();
    if (palinkStatus && online_status == 'no_connection') {
        online_status = palinkStatus;
    }
...
export async function selectCharacterById(id, { switchMenu = true } = {}) {
    if (globalThis.__PALINK_ST_SUPPRESS_CHARACTER_EDIT && switchMenu === true) {
        switchMenu = false;
    }
...
export function setOnlineStatus(value) {
    if (value === 'no_connection') {
        value = getPalinkManagedApiStatus() || value;
    }
...
export function getThumbnailUrl(type, file, t = false) {
    const token = globalThis.localStorage?.getItem('palink_token') || '';
    const tokenPart = token ? `&token=${encodeURIComponent(token)}` : '';
    return `/api/st/thumbnail?type=${type}&file=${encodeURIComponent(file)}${tokenPart}${t ? `&t=${Date.now()}` : ''}`;
}
...
async function getChatResult() {
    ...
    await printMessages();
    select_selected_character(this_chid, { switchMenu: !globalThis.__PALINK_ST_SUPPRESS_CHARACTER_EDIT });
...
export async function getSettings(initLoaderHandle = null) {
    ...
    const data = await response.json();
    data.koboldai_settings = data.koboldai_settings || [];
    data.koboldai_setting_names = data.koboldai_setting_names || [];
    data.novelai_settings = data.novelai_settings || [];
    data.novelai_setting_names = data.novelai_setting_names || [];
    data.textgenerationwebui_presets = data.textgenerationwebui_presets || [];
    data.textgenerationwebui_preset_names = data.textgenerationwebui_preset_names || [];
    data.openai_settings = data.openai_settings || [];
    data.openai_setting_names = data.openai_setting_names || [];
