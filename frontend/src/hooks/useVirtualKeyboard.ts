...
export const useVirtualKeyboard = () => {
  ...
  useEffect(() => {
    ...
    const initHeight = getCurrentViewportHeight();
    baselineViewportHeightRef.current = Math.max(initHeight, window.innerHeight);
    ...
    const commitKeyboardState = () => {
      ...
      const open = focused && nextHeight > 24;

      if (!open) {
        baselineViewportHeightRef.current = Math.max(baselineViewportHeightRef.current, currentViewportHeight, window.innerHeight);
      }
