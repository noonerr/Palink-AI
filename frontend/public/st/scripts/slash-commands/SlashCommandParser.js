...
export class SlashCommandParser {
    ...
    static addCommandObjectUnsafe(command) {
        ...
        command.isExtension = stack.find(it => it.includes('/st/scripts/extensions/')) != null;
        command.isThirdParty = stack.find(it => it.includes('/st/scripts/extensions/third-party/')) != null;
        if (command.isThirdParty) {
            command.source = stack.find(it => it.includes('/st/scripts/extensions/third-party/')).replace(/^.*?\/st\/scripts\/extensions\/third-party\/([^/]+)\/.*$/, '$1');
        } else if (command.isExtension) {
            command.source = stack.find(it => it.includes('/st/scripts/extensions/')).replace(/^.*?\/st\/scripts\/extensions\/([^/]+)\/.*$/, '$1');
        } else {
