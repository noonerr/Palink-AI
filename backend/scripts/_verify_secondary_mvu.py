"""验证副 AI 服务：用真实角色 schema + 真实剧情，测副 AI 能否生成变量 patches。

用法：docker exec palink-ai-backend-1 python /app/scripts/_verify_secondary_mvu.py
"""
...
from app.services.mvu_secondary import run_secondary_mvu  # noqa: E402

CHAR_ID = "c21c3512-7d38-4177-a55f-742afabe5ca6"
USER_ID = 1
SECONDARY_MODEL = "deepseek-v4-flash"  # 测试用主模型当副模型

# 真实剧情（2219 的正文，无 UpdateVariable 块）
...
async def main() -> None:
    ...
    assert char is not None, "角色不存在"

    # 构建 char_ext_raw（extensions + worldbook 合并）
    char_ext_raw = {}
    try:
    ...
    print(f"[INFO] schema 默认值 keys: {list(schema_defaults.keys()) if schema_defaults else '无'}")
    ...
    print(f"[INFO] 副 AI 生成 {len(patches)} 个 patches")
    for p in patches:
        print(f"  patch: {p}")

    if patches:
        new_vars, applied = apply_patches(init, patches)
        print(f"[INFO] 应用后变更日志: {applied}")
        ...
        print("[PASS] 副 AI 变量更新链路正常")
    else:
        print("[WARN] 副 AI 未生成 patches（需检查副模型输出）")
