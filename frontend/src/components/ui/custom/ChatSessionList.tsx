...
function SessionTitleText({ title }: { title: string }) {
  const formattedTitle = useMemo(() => formatSessionTitle(title), [title]);
  const hasMath = useMemo(() => /\$[^$]+\$/.test(title), [title]);
...
export const ChatSessionList = React.memo<ChatSessionListProps>(({
  ...
  const sessionLabels = useMemo(() => {
    const defaultLabel = t?.new_chat || '新对话';
    return new Map(sessions.map(s => [s.id, formatSessionTitle(s.title || defaultLabel)]));
  }, [sessions, t?.new_chat]);
