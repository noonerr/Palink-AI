...
type SubTab = 'providers' | 'local' | 'routing' | 'web_search' | 'mcp' | 'tts';
...
export function ModelManagementTab({
  ...
  const [currentTtsVoice, setCurrentTtsVoice] = useState<{ voice_id: string; gender: string }>({
    voice_id: '',
    gender: 'female'
  });
  const [ttsProviders, setTtsProviders] = useState<any[]>([]);
  const [ttsActiveProviderId, setTtsActiveProviderId] = useState<string>('browser');
  const [ttsEditingProvider, setTtsEditingProvider] = useState<any>(null);
  const [ttsBuiltinConfigEdits, setTtsBuiltinConfigEdits] = useState<Record<string, Record<string, string>>>({});

  const fetchTtsData = useCallback(async () => {
    try {
      const [providersData, voicesData, voiceData] = await Promise.all([
        api.tts.getProviders(),
        api.tts.getVoices(),
        api.tts.getMyVoice(),
      ]);
      setTtsProviders(providersData.providers || []);
      setTtsActiveProviderId(providersData.active_provider_id || 'browser');
      setTtsVoices(voicesData.voices || []);
      setCurrentTtsVoice(voiceData);
    } catch (e) {
  ...
  const startEditModel = (model: UnifiedModel) => {
    ...
    setEditForm({
      ...
      context_length: model.context_length != null ? String(model.context_length) : '',
      max_output_tokens: model.max_output_tokens != null ? String(model.max_output_tokens) : '',
      provider_overrides: overrides,
    });
  };

  // 直接解析为 token 整数：去除千分位逗号后按原值存储，不做 K/M 换算
  const parseTokenInput = (v: string): number | null => {
  ...
  const saveModelConfig = async (unifiedId: string) => {
    try {
      const ctx = parseTokenInput(editForm.context_length);
      const out = parseTokenInput(editForm.max_output_tokens);
      const payload: any = { ...editForm };
      if (ctx != null) payload.context_length = ctx;
      else delete payload.context_length;
      if (out != null) payload.max_output_tokens = out;
      else delete payload.max_output_tokens;
      await api.put(`${API_BASE}/${encodeURIComponent(unifiedId)}`, payload);
      invalidateCache('/api/models');
  ...
  // TTS 相关函数
  const handleSaveTtsVoice = async (voiceId: string, gender: string) => {
    try {
      await api.tts.setVoice(voiceId, gender);
      setCurrentTtsVoice({ voice_id: voiceId, gender });
  ...
  return (
    <div className="flex flex-col gap-6 animate-fade-in">
        ...
        <div className="space-y-6 animate-fade-in pb-28 w-full max-w-full">
            <div className="space-y-4">
              <div className="flex justify-between items-center">
                <h3 className="text-2xl font-semibold hidden md:block">{t.provider_config || '服务提供商配置'}</h3>
                            ...
                            <>
                              <div className="space-y-3">
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                  <div>
                                    <label className="text-xs font-medium text-muted-foreground mb-1 block">上下文上限 (tokens)</label>
                                    <input
                                      type="text"
                                      placeholder="如 128000"
                                      value={editForm.context_length}
                                      onChange={(e) => setEditForm((prev) => ({ ...prev, context_length: e.target.value }))}
                                      className="w-full px-3 py-2 rounded-lg border border-border bg-background text-sm"
                                    />
                                  </div>
                                  <div>
                                    <label className="text-xs font-medium text-muted-foreground mb-1 block">最大输出 (tokens)</label>
                                    <input
                                      type="text"
                                      placeholder="如 8000"
                                      value={editForm.max_output_tokens}
                                      onChange={(e) => setEditForm((prev) => ({ ...prev, max_output_tokens: e.target.value }))}
                                      className="w-full px-3 py-2 rounded-lg border border-border bg-background text-sm"
                                    />
                                  </div>
                                </div>
                                <p className="text-[11px] text-muted-foreground">
                                  直接填写 token 数（例如 128000）。留空则按系统清单自动识别。
                                </p>
                ...
                <div className="flex gap-2">
                  ...
                  <Button onClick={async () => {
                    try {
                      ...
                      invalidateCache('/api/admin/web-search');
                      setWsTestResult(null);
                    } catch (e) {
        ...
        <div className="space-y-6 animate-fade-in pb-28 w-full max-w-full">
            <div className="space-y-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-2xl font-semibold">语音合成设置</h3>
            ...
            <GlassCard className="p-4 sm:p-5">
              ...
              <div className="space-y-3">
                {ttsProviders.length > 0 ? ttsProviders.map((provider: any) => (
                  <div
                    key={provider.id}
                    className={cn(
                      "p-4 rounded-lg border-2 transition-all",
                      ttsActiveProviderId === provider.id
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          {provider.is_builtin && (
                            <span className="text-[10px] bg-primary/10 text-primary px-1.5 py-0.5 rounded-full">内置</span>
                          )}
                          {!provider.is_builtin && (
                            <span className="text-[10px] bg-amber-500/15 text-amber-500 px-1.5 py-0.5 rounded-full">自定义</span>
                          )}
                          {ttsActiveProviderId === provider.id && (
                            <span className="text-[10px] bg-green-500/15 text-green-500 px-1.5 py-0.5 rounded-full">当前使用</span>
                          )}
                        </div>
                        <p className="text-xs text-muted-foreground">{provider.description}</p>
                        {provider.engine_type && (
                          <p className="text-[10px] text-muted-foreground/60 mt-1">引擎: {provider.engine_type}</p>
                        )}
                      </div>
                      <div className="flex items-center gap-2 shrink-0 ml-3">
                        {ttsActiveProviderId !== provider.id && (
                          <Button size="sm" variant="outline" onClick={async () => {
                            try {
                              await api.tts.saveConfig({ active_provider_id: provider.id });
                              setTtsActiveProviderId(provider.id);
                              const voicesData = await api.tts.getVoices();
                              setTtsVoices(voicesData.voices || []);
                              toast.success(`已切换到 ${provider.name}`);
                            } catch (e: any) {
                              toast.error(e?.detail || '切换失败');
                            }
                          }}>
                            启用
                          </Button>
                        )}
                        {!provider.is_builtin && isAdmin && (
                          <>
                            <Button size="sm" variant="ghost" onClick={() => {
                              setTtsEditingProvider({
                                ...
                                engine_type: provider.engine_type || 'custom_api',
                            ...
                            <Button size="sm" variant="ghost" className="text-destructive hover:bg-destructive/10" onClick={async () => {
                              try {
                                await api.tts.deleteProvider(provider.id);
                                await fetchTtsData();
                                toast.success('已删除');
                              } catch (e: any) {
                    ...
                    {/* 服务商配置字段 */}
                    {provider.config_fields && provider.config_fields.length > 0 && (
                      <div className="mt-3 pt-3 border-t border-border/50">
                        <div className="space-y-2">
                          {provider.config_fields.map((field: any) => (
                            <div key={field.key} className="flex items-center gap-2">
                              ...
                              {field.type === 'password' ? (
                                <div className="flex-1 flex items-center gap-2">
                                  <Input
                                    type="password"
                                    value={ttsBuiltinConfigEdits?.[provider.id]?.[field.key] ?? ''}
                                    onChange={e => {
                                      const current = ttsBuiltinConfigEdits?.[provider.id] ?? {};
                                      const newEdits = { ...ttsBuiltinConfigEdits, [provider.id]: { ...current, [field.key]: e.target.value } };
                                      setTtsBuiltinConfigEdits(newEdits);
                                    }}
                                    placeholder={provider.config?.[field.key] ? '已配置（留空则保持不变）' : (field.placeholder || '未设置')}
                                    className="h-7 text-xs flex-1"
                                  />
                                  <Button size="sm" variant="outline" className="h-7 text-xs shrink-0" onClick={async () => {
                                    const val = ttsBuiltinConfigEdits?.[provider.id]?.[field.key];
                                    if (val === undefined || val === '') return;
                                    try {
                                      await api.tts.saveConfig({ provider_configs: { [provider.id]: { [field.key]: val } } });
                                      const newEdits = { ...ttsBuiltinConfigEdits };
                                      delete newEdits[provider.id];
                                      setTtsBuiltinConfigEdits(newEdits);
                                      await fetchTtsData();
                                      toast.success('配置已保存');
                                    } catch (e: any) {
                                ...
                                <div className="flex-1 flex items-center gap-2">
                                  <Input
                                    value={ttsBuiltinConfigEdits?.[provider.id]?.[field.key] ?? (provider.config?.[field.key] || '')}
                                    onChange={e => {
                                      const current = ttsBuiltinConfigEdits?.[provider.id] ?? {};
                                      const newEdits = { ...ttsBuiltinConfigEdits, [provider.id]: { ...current, [field.key]: e.target.value } };
                                      setTtsBuiltinConfigEdits(newEdits);
            ...
            {/* 语音选择 */}
            <GlassCard className="p-4 sm:p-5">
              <h4 className="text-lg font-semibold mb-4">语音选择</h4>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {ttsVoices.length > 0 ? ttsVoices.map((voice: any) => (
                  <button
                    key={voice.voice_id}
                    onClick={() => handleSaveTtsVoice(voice.voice_id, voice.gender)}
                    className={cn(
                      "p-3 rounded-lg border-2 text-left transition-all hover:shadow-md",
                      currentTtsVoice.voice_id === voice.voice_id
                        ? "border-primary bg-primary/5"
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <div className="text-xs text-muted-foreground mt-0.5">
                          {voice.gender === 'female' ? '女声' : '男声'} · {voice.voice_id}
                        </div>
                      </div>
                      {currentTtsVoice.voice_id === voice.voice_id && (
            ...
            {/* 提示 */}
            <GlassCard className="p-4 sm:p-5">
              <h5 className="text-sm font-medium mb-2">使用说明</h5>
              <ul className="text-xs text-muted-foreground space-y-1">
                <li>• 在对话中点击消息旁的音量图标来播放语音</li>
                <li>• 模型会根据对话内容自动选择合适的语音风格</li>
                <li>• 「浏览器内置」无需配置，但音质取决于系统</li>
                <li>• 「小米 MIMO TTS」免费使用，音质更好</li>
                <li>• 「自定义 TTS API」支持兼容 OpenAI 接口格式的服务</li>
              </ul>
            </GlassCard>
        </div>
      )}
      ...
      {/* TTS 自定义服务商编辑弹窗 */}
      {ttsEditingProvider && createPortal(
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={() => setTtsEditingProvider(null)}>
          <div className="bg-background rounded-xl shadow-2xl w-full max-w-lg mx-4 max-h-[80vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="p-5 border-b border-border/50">
              <h3 className="text-lg font-semibold">{ttsEditingProvider.id ? '编辑' : '添加'} TTS 服务商</h3>
            </div>
            <div className="p-5 space-y-4">
              <div>
                <label className="text-sm font-medium block mb-1">服务商 ID</label>
                <Input
                  value={ttsEditingProvider.id}
                  onChange={e => setTtsEditingProvider({ ...ttsEditingProvider, id: e.target.value.replace(/[^a-zA-Z0-9_]/g, '') })}
                  placeholder="my_tts_provider"
                  disabled={!!ttsEditingProvider.id && ttsProviders.some((p: any) => p.id === ttsEditingProvider.id)}
                />
                <p className="text-[10px] text-muted-foreground mt-1">唯一标识，创建后不可修改</p>
              </div>
              <div>
                <label className="text-sm font-medium block mb-1">名称</label>
                <Input
                  value={ttsEditingProvider.name}
                  onChange={e => setTtsEditingProvider({ ...ttsEditingProvider, name: e.target.value })}
              ...
              <div>
                ...
                <Input
                  value={ttsEditingProvider.description}
                  onChange={e => setTtsEditingProvider({ ...ttsEditingProvider, description: e.target.value })}
                  placeholder="自定义 TTS 服务"
                />
              </div>
              <div>
                <label className="text-sm font-medium block mb-1">引擎类型</label>
                <select
                  value={ttsEditingProvider.engine_type}
                  onChange={e => setTtsEditingProvider({ ...ttsEditingProvider, engine_type: e.target.value })}
              ...
              {/* API 配置 */}
              {(ttsEditingProvider.engine_type === 'custom_api' || ttsEditingProvider.engine_type === 'xiaomi_mimo') && (
                <div className="space-y-3 p-3 rounded-lg bg-muted/30">
                  <h5 className="text-sm font-medium">API 配置</h5>
                  {ttsEditingProvider.engine_type === 'custom_api' && (
                    <div>
                      <label className="text-xs text-muted-foreground block mb-1">API 地址 *</label>
                      <Input
                        value={ttsEditingProvider.config?.base_url || ''}
                        onChange={e => setTtsEditingProvider({
                          ...ttsEditingProvider,
                          config: { ...ttsEditingProvider.config, base_url: e.target.value }
                        })}
                  ...
                  <div>
                    ...
                    <Input
                      type="password"
                      value={ttsEditingProvider.config?.api_key || ''}
                      onChange={e => setTtsEditingProvider({
                        ...ttsEditingProvider,
                        config: { ...ttsEditingProvider.config, api_key: e.target.value }
                      })}
                      placeholder="可选"
                    />
                  </div>
                  {ttsEditingProvider.engine_type === 'custom_api' && (
                    <div>
                      ...
                      <Input
                        value={ttsEditingProvider.config?.model || ''}
                        onChange={e => setTtsEditingProvider({
                          ...ttsEditingProvider,
                          config: { ...ttsEditingProvider.config, model: e.target.value }
                        })}
                        placeholder="tts-1"
                      />
                    </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <Button size="sm" variant="outline" onClick={() => {
                    const voices = [...(ttsEditingProvider.voices || [])];
                    voices.push({ voice_id: `voice_${voices.length + 1}`, gender: 'female', description: '新语音' });
                    setTtsEditingProvider({ ...ttsEditingProvider, voices });
                  }}>
                    <Plus size={12} className="mr-1" /> 添加语音
                  </Button>
                </div>
                <div className="space-y-2">
                  {(ttsEditingProvider.voices || []).map((voice: any, idx: number) => (
                    <div key={idx} className="flex items-center gap-2 p-2 rounded border border-border/50">
                      <Input
                        value={voice.voice_id}
                        onChange={e => {
                          const voices = [...ttsEditingProvider.voices];
                          voices[idx] = { ...voices[idx], voice_id: e.target.value };
                          setTtsEditingProvider({ ...ttsEditingProvider, voices });
                        }}
                      ...
                      <select
                        ...
                        onChange={e => {
                          const voices = [...ttsEditingProvider.voices];
                          voices[idx] = { ...voices[idx], gender: e.target.value };
                          setTtsEditingProvider({ ...ttsEditingProvider, voices });
                        }}
                      ...
                      <Input
                        ...
                        onChange={e => {
                          const voices = [...ttsEditingProvider.voices];
                          voices[idx] = { ...voices[idx], description: e.target.value };
                          setTtsEditingProvider({ ...ttsEditingProvider, voices });
                        }}
                        placeholder="描述"
                        className="flex-1 h-8 text-xs"
                      ...
                      <Button size="sm" variant="ghost" className="h-8 w-8 p-0 text-destructive" onClick={() => {
                        const voices = ttsEditingProvider.voices.filter((_: any, i: number) => i !== idx);
                        setTtsEditingProvider({ ...ttsEditingProvider, voices });
                      }}>
                        <Trash2 size={12} />
                      </Button>
            ...
            <div className="p-5 border-t border-border/50 flex justify-end gap-2">
              <Button variant="outline" onClick={() => setTtsEditingProvider(null)}>取消</Button>
              <Button onClick={async () => {
                if (!ttsEditingProvider.id || !ttsEditingProvider.name) {
                  toast.error('请填写服务商 ID 和名称');
                  return;
                }
                try {
                  const isEdit = ttsProviders.some((p: any) => p.id === ttsEditingProvider.id);
                  if (isEdit) {
                    await api.tts.updateProvider(ttsEditingProvider.id, ttsEditingProvider);
                  } else {
                    await api.tts.addProvider(ttsEditingProvider);
                  }
                  setTtsEditingProvider(null);
                  await fetchTtsData();
