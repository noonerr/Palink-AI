...
const testContentWithEscaped = `&lt;|a|&gt;这是转义的动作标签&lt;|/a|&gt; 普通内容 &lt;|t|&gt;这是转义的思考标签&lt;|/t|&gt;`;
...
function parseContent(content) {
    ...
    console.log('解码后内容:', remaining.substring(0, 200));
    ...
    for (const { type, pattern } of tagPatterns) {
        ...
        while ((match = pattern.exec(remaining)) !== null) {
            console.log(`匹配到标签: ${type}, 位置: ${match.index}`);
            matches.push({
                type,
                start: match.index,
...
// 测试1: 原始内容
console.log('=== 测试1: 原始内容 ===');
const result1 = parseContent(testContent);
console.log('解析到的部分数量:', result1.length);
result1.forEach((part, index) => {
    console.log(`\n部分 ${index + 1}:`);
    console.log(`  类型: ${part.type}`);
    console.log(`  内容: ${part.content.substring(0, 50)}...`);
});

// 测试2: 转义内容
console.log('\n\n=== 测试2: 转义内容 ===');
const result2 = parseContent(testContentWithEscaped);
console.log('解析到的部分数量:', result2.length);
result2.forEach((part, index) => {
    console.log(`\n部分 ${index + 1}:`);
    console.log(`  类型: ${part.type}`);
    console.log(`  内容: ${part.content}`);
});
