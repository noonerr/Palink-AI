...
class StreamSessionBusyError(Exception):
    """M-7 修复: 同会话已有 active 生成任务时抛出。

    调用方（websocket.py）捕获后应回滚刚落库的用户消息并向前端
    发送明确错误提示，避免「用户消息已落库但 AI 回复被静默丢弃」。
    """

    def __init__(self, session_id: str):
        super().__init__(f"Session {session_id} already has an active generation task")
...
class WebSocketManager:
    ...
    def __init__(self):
        ...
        # Task 3.4.4/3.4.5: 等待前端 tool_call_response 的队列，按 session_id 索引
        # 每个 session 一个队列，tool_executor 通过它等待前端执行结果
        self._pending_tool_responses: Dict[str, "asyncio.Queue[Dict[str, Any]]"] = {}
    ...
    async def create_stream_session(
        ...
        async with self._stream_lock:
            ...
            if existing and existing.is_active:
                # M-7 修复: 不再静默返回 existing——抛出明确异常，让调用方
                # 回滚刚落库的用户消息并通知前端（否则用户消息存在但无回复，
                # 前端无任何错误提示）。
    ...
    # ── Task 3.4.4/3.4.5: function tool 前端执行支持 ──────────────────────
    async def request_tool_call(self, session_id: str, tool_call_id: str, name: str, arguments: Any) -> str:
        """通过 WebSocket 向前端请求执行 function tool，阻塞等待响应。

        tool_executor 回调会调用此方法。它发送 tool_call_request 给前端，
        然后从该 session 的队列等待对应的 tool_call_response。
        """
        # 确保队列存在（懒创建）
        if session_id not in self._pending_tool_responses:
            self._pending_tool_responses[session_id] = asyncio.Queue()
        queue = self._pending_tool_responses[session_id]
        ...
        try:
            response = await asyncio.wait_for(queue.get(), timeout=60.0)
        except asyncio.TimeoutError:
            return "Tool error: frontend did not respond within 60s"
        return response.get("result", "")

    def submit_tool_response(self, session_id: str, tool_call_id: str, result: Any) -> bool:
        """前端通过 WebSocket 发送 tool_call_response 时调用，放入队列。

        返回 True 表示成功投递，False 表示没有等待中的队列（如已超时）。
        """
        queue = self._pending_tool_responses.get(session_id)
        if queue is None:
            return False
        try:
            queue.put_nowait({"tool_call_id": tool_call_id, "result": result})
            return True
        except Exception:
            return False
