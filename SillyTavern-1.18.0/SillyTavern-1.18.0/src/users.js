...
import { checkForNewContent, getContentOfType } from './endpoints/content-manager.js';
...
import { ensurePalinkNativeDirectories, getPalinkNativeRequestProfile, seedPalinkNativeUserDefaults } from './palink-native.js';
...
export async function setUserDataMiddleware(request, response, next) {
    const palinkProfile = getPalinkNativeRequestProfile(request, DEFAULT_USER);
    if (palinkProfile) {
        const directories = getUserDirectories(palinkProfile.handle);
        try {
            ensurePalinkNativeDirectories(directories);
            if (!fs.existsSync(path.join(directories.root, 'content.log'))) {
                await checkForNewContent([directories]);
            }
            seedPalinkNativeUserDefaults(directories.root);
        } catch (error) {
            console.warn(`Failed to initialize Palink ST Native user ${palinkProfile.handle}:`, error);
        }
        request.user = {
            profile: palinkProfile,
