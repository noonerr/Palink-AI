...
const getProcessingText = (char: Character) => {
  if (char.processing_status && !char.processing_status.includes('完成') && !char.processing_status.includes('重置')) {
    return char.processing_status;
  }
  return '处理中...';
};
...
export const CharacterList: React.FC<CharacterListProps> = ({
  ...
  const headerClass = 'h-[54px] flex items-center justify-between px-6 border-b z-10 flex-shrink-0';
  ...
  return (
    <div className="flex-1 flex flex-col w-full h-full">
          ...
          {characters.length > 0 && (
            <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 sm:gap-4 md:gap-6">
              {characters.map((char) => (
                <div
                  key={char.id}
                  className="group relative border rounded-2xl transition-all cursor-pointer overflow-hidden bg-card border-border hover:border-border/80 hover:shadow-md"
                ...
                >
                  <div className="aspect-[3/4] sm:aspect-[4/5] bg-gradient-to-br from-gray-200 to-gray-300 dark:from-gray-700 dark:to-gray-800 relative overflow-hidden">
                    ...
                    {(processingCharacter === char.id || forceShowOverlay === char.id) && (
                      <div className="absolute inset-0 bg-black/80 flex items-center justify-center z-50">
                        <div className="text-white text-center p-8">
                          ...
                          <p className="text-lg mb-6 font-semibold">{getProcessingText(char)}</p>
                          <Button
                            variant="destructive"
                            size="default"
                    ...
                    <div className="absolute inset-0 bg-gradient-to-t from-card/95 via-card/40 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-200" />
                  ...
                  <div className="p-2 sm:p-3">
                    <h3 className="font-semibold text-sm sm:text-base truncate">{char.name}</h3>
                    {char.is_processing && (
                      <p className="text-xs text-muted-foreground truncate">{getProcessingText(char)}</p>
                    )}
                  </div>
                </div>
              ))}
      ...
      {showImportOptions && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center z-[80]">
          <div className="bg-card border border-border rounded-xl p-6 max-w-md w-full mx-4">
            <h3 className="text-lg font-semibold mb-4">导入成功，下一步操作</h3>
            <div className="space-y-2">
              ...
              <Button variant="ghost" className="w-full" onClick={() => onSetShowImportOptions(null)}>
                跳过
              </Button>
            </div>
          </div>
      ...
      {showProcessingMessage.show && (
        <div className="fixed top-4 right-4 bg-card border border-border rounded-xl p-4 shadow-xl z-[80] flex items-center gap-3 animate-slide-in-right">
