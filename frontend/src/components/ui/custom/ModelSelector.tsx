...
export function ModelSelector({
  ...
  const getProviderLabel = (model: Model): string => {
    if (!model.provider) return '';
    if (model.provider === 'local') return '本地';
    return model.provider;
  };

  // 触发按钮下方的小字提供商行（如 "opencode go · 2 个提供商"）
  const getProviderSubtext = (model: Model): string => {
    const base = getProviderLabel(model);
    if (!base) return '';
    if (model.provider_count && model.provider_count > 1) {
      return `${base} · ${model.provider_count} 个提供商`;
    }
    return base;
  };

  // 两行式触发标签：上行为模型名，下行为小字提供商
  const renderTriggerLabel = (
    ...
    const sub = getProviderSubtext(model || ({} as Model));
    return (
      <div className={cn('flex flex-col items-start leading-tight min-w-0', containerClass)}>
        <span className={cn(nameClass, 'truncate w-full')}>{name}</span>
        {showProvider && sub && (
        ...
        >
          ...
          <div className="flex-1 text-left min-w-0">
            ...
            <div className="text-[10px] text-muted-foreground flex items-center gap-1 mt-0.5">
              <span className="truncate">{getProviderSubtext(model)}</span>
            </div>
          </div>
  ...
  if (size === 'sm') {
    if (triggerStyle === 'mobile-bar') {
      return (
        <div ref={containerRef} className="relative">
          ...
          >
            {currentModelIcon}
            {renderTriggerLabel(currentModelObj, 'text-sm font-medium', true, 'max-w-[120px]')}
            <ChevronDown
              size={16}
      ...
      return (
        <div ref={containerRef} className="relative">
          <button
            ...
            onClick={() => setIsOpen(!isOpen)}
            className={cn(
              'flex h-10 items-center gap-2 px-3.5 rounded-[30px] border backdrop-blur-[30px] transition-all duration-300 ease-in-out',
              isDarkTheme
                ? 'border border-[rgba(255,255,255,0.18)] bg-[rgba(15,23,42,0.5)] text-white'
                : 'border-[#ddd4c5] bg-[rgba(255,250,250,0.7)] text-slate-700',
              isOpen && (isDarkTheme ? 'ring-2 ring-slate-500/70' : 'ring-2 ring-[#d7cab2]')
    ...
    return (
      <div ref={containerRef} className="relative">
        ...
        >
          {currentModelIcon}
          {renderTriggerLabel(currentModelObj, 'text-xs font-medium', true, 'max-w-[80px] sm:max-w-[110px]')}
          <ChevronDown
            size={14}
  ...
  return (
    <div ref={containerRef} className="relative">
      ...
      >
        <Bot size={16} />
        {renderTriggerLabel(currentModelObj, 'text-sm font-medium', true, 'max-w-[180px]')}
        <ChevronDown
          size={14}
          className={cn("transition-transform", isOpen && "rotate-180")}
        />
