...
def _public_local_model(model: dict) -> dict:
    safe_model = dict(model)
    safe_model.pop("path", None)
    safe_model.pop("mmproj_path", None)
    return safe_model


def _public_unified_model(model: dict) -> dict:
    safe_model = dict(model)
    safe_providers = []
    for provider in model.get("providers", []):
        ...
        for key in (
            ...
            safe_provider.pop(key, None)
        safe_providers.append(safe_provider)
    safe_model["providers"] = safe_providers
    return safe_model
...
@cached(ttl_seconds=5, key_prefix="models")
async def get_models(user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    """获取所有启用服务商的可用模型列表"""
    result = []
    seen_ids: Set[str] = set()

    # 走 unified 模型表：合并同名模型 + 尊重 unified_models.json 的 provider_overrides。
    # 这样在「模型管理」里按模型禁用某个 provider 会在聊天选择器生效，
    # 且每个模型暴露 providers 列表，供前端为同名模型单独选择 A / B。
    for m in get_flat_model_list():
        model_id = str(m.get("id") or "")
        if model_id and model_id not in seen_ids:
            seen_ids.add(model_id)
            result.append({
                "id": m["id"],
                "name": m.get("name") or m.get("alias") or m["id"],
                "alias": m.get("alias") or m.get("name") or m["id"],
                "icon": m.get("icon", "🤖"),
                ...
                "provider_count": m.get("provider_count", 1),
...
async def get_local_models(all: bool = Query(False), user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if _is_admin(user):
        return models
    return [_public_local_model(model) for model in models]


@router.get("/api/models/unified")
async def get_unified_models(user: User = Depends(get_current_user)):
    """获取统一模型列表（含多提供商信息）"""
    models = get_unified_model_list()
    if _is_admin(user):
        return models
    return [_public_unified_model(model) for model in models]
...
async def get_model_routing_strategies(user: User = Depends(get_admin)):
    """获取可用的路由策略列表"""
...
async def update_unified_model_config(
    ...
    """更新统一模型配置（显示名、路由策略、提供商优先级、上下文/输出上限等）"""
