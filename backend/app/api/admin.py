...
from ..core.auth_config import get_auth_config, save_auth_config
...
async def get_providers_api(user: User = Depends(get_admin)):
    ...
    # 将 provider 内每个模型的 ctx / max_output 解析为真实值（model_specs + 用户覆盖），
    # 供前端「模型管理 → 服务商 → 模型列表」正确展示，而非显示 providers.json 默认 4096。
    from ..services.unified_model_registry import resolve_model_token_limits, get_model_spec
    for p in providers:
        for m in p.get("models", []):
            if isinstance(m, dict) and m.get("id"):
                ctx, out = resolve_model_token_limits(
                    m["id"], m.get("context_length"), m.get("max_output_tokens")
                )
                m["context_length"] = ctx
                m["max_output_tokens"] = out
                # 用清单默认值补全能力标记：仅在 provider 未显式开启时填入，
                # 用户手动开启的开关优先保留（手动关闭会回退到清单默认，符合「默认配置」语义）。
                # 注：深度思考不再由模型级开关控制，统一交给对话/角色设置里的「思考强度」(reasoning_effort)。
                spec = get_model_spec(m["id"]) or {}
                if not m.get("supports_vision"):
                    m["supports_vision"] = bool(spec.get("supports_vision", False))
    return providers
...
async def get_system_defaults(user: User = Depends(get_admin), db: Session = Depends(get_db)):
...
async def sync_provider_models(
    ...
    """Fetch model list from a provider's API and return them."""
    try:
        saved_base_urls = {p.get("base_url", "").rstrip("/") for p in _get_providers() if p.get("base_url")}
        req_base_url = (req.base_url or "").rstrip("/")
        is_trusted_url = req_base_url in saved_base_urls
        ...
        if _is_env_secret_ref(req.api_key) and not is_trusted_url:
            raise ValueError(
                "Cannot resolve environment variable secrets for untrusted base_url. "
                "Save the provider first, then sync with its saved base_url."
            )
        ...
        for m in (models_resp.data or []):
            # 优先采用 API 返回的上下文窗口（部分提供商支持），否则回退默认 4096
            api_ctx = None
            try:
                api_ctx = getattr(m, "context_length", None)
            except Exception:
                api_ctx = None
            if not isinstance(api_ctx, int) or api_ctx <= 0:
                api_ctx = 4096
            model_list.append({
                "id": m.id,
                "name": m.id,
        ...
        return {"success": False, "models": [], "count": 0, "message": str(e)}
...
async def get_admin_auth_config(user: User = Depends(get_admin), db: Session = Depends(get_db)):
    ...
    from ..services.oauth_service import get_oauth_providers_config, decrypt_secret, PROVIDER_PRESETS
    providers = get_oauth_providers_config(db)
    result_providers = []
    for p in providers:
        item = dict(p)
        if item.get("client_secret"):
            decrypted = decrypt_secret(item["client_secret"])
            item["client_secret"] = decrypted[:4] + "****" if len(decrypted) > 4 else "****"
        preset = PROVIDER_PRESETS.get(item.get("name", ""), {})
        item.setdefault("authorize_url", preset.get("authorize_url", ""))
        item.setdefault("token_url", preset.get("token_url", ""))
        item.setdefault("userinfo_url", preset.get("userinfo_url", ""))
        item.setdefault("scopes", preset.get("scopes", ""))
        result_providers.append(item)
    config["oauth_providers"] = result_providers
    return config
...
async def save_admin_auth_config(
    ...
    save_auth_config(db, config.model_dump())
    return {"status": "ok"}
...
async def get_oauth_providers(user: User = Depends(get_admin), db: Session = Depends(get_db)):
    from ..services.oauth_service import get_oauth_providers_config, decrypt_secret, PROVIDER_PRESETS
    providers = get_oauth_providers_config(db)
    result = []
    for p in providers:
...
async def save_oauth_providers(
    providers: List[OAuthProviderConfig],
    user: User = Depends(get_admin),
    db: Session = Depends(get_db),
):
    from ..services.oauth_service import save_oauth_providers_config, get_oauth_providers_config, decrypt_secret
    existing = get_oauth_providers_config(db)
    existing_map = {p.get("name"): p for p in existing}

    data = []
    for p in providers:
        item = p.model_dump()
        if item.get("client_secret") and item["client_secret"].endswith("****"):
            old = existing_map.get(item["name"], {})
            item["client_secret"] = old.get("client_secret", "")
        data.append(item)

    save_oauth_providers_config(db, data)
    return {"status": "ok"}


@router.delete("/oauth/providers/{provider}")
async def delete_oauth_provider(provider: str, user: User = Depends(get_admin), db: Session = Depends(get_db)):
    from ..services.oauth_service import get_oauth_providers_config, save_oauth_providers_config
    providers = get_oauth_providers_config(db)
    original_len = len(providers)
    updated = [p for p in providers if p.get("name") != provider]
    if len(updated) == original_len:
        raise HTTPException(status_code=404, detail="OAuth provider not found")
    save_oauth_providers_config(db, updated)
    return {"status": "ok"}


@router.post("/oauth/providers/{provider}/test")
async def test_oauth_provider(provider: str, user: User = Depends(get_admin), db: Session = Depends(get_db)):
    from ..services.oauth_service import get_oauth_provider_config, build_authorize_url, PROVIDER_PRESETS
    config = get_oauth_provider_config(provider, db)
    if not config:
        preset = PROVIDER_PRESETS.get(provider)
        if not preset:
            return {"success": False, "message": f"OAuth provider '{provider}' not configured"}
        return {"success": False, "message": f"OAuth provider '{provider}' is not enabled"}

    try:
        import httpx
        async with httpx.AsyncClient(timeout=10.0) as client:
            resp = await client.get(config.get("authorize_url", ""))
            if resp.status_code < 400:
                return {"success": True, "message": f"Authorization endpoint reachable ({resp.status_code})"}
            else:
                return {"success": False, "message": f"Authorization endpoint returned {resp.status_code}"}
    except Exception as e:
