...
start_sillytavern() {
    ...
    # Seed Palink-specific defaults for a fresh ST Native data root without
    # overwriting an existing SillyTavern user's settings or secrets.
    if [ -f "./palink-st-native-init.mjs" ]; then
        $PREFIX node "./palink-st-native-init.mjs"
    fi
