...
def _create_vision_chat_handler(mmproj_path: str) -> Any:
    try:
        ...
        logger.warning("Failed to create vision chat handler for mmproj %s: %s", os.path.basename(mmproj_path), e)
        return None
    ...
    def _create_llm(self, model_path: str, mmproj_path: Optional[str] = None) -> Any:
        ...
        if mmproj_path and os.path.isfile(mmproj_path):
            ...
            if chat_handler is not None:
                ...
                logger.info(
                    "Loading llama.cpp model with vision chat_handler: %s, mmproj: %s",
                    os.path.basename(model_path),
                    os.path.basename(mmproj_path),
                )
            else:
                logger.warning(
                    "Vision chat_handler creation failed, loading model without vision support: %s",
                    os.path.basename(model_path),
                )
        else:
            logger.info("Loading llama.cpp model: %s", os.path.basename(model_path))
