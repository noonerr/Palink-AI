...
import { debounceAsync, debug, log, warn, quickReplyApi, setQuickReplyApi, getQuickReplyApi } from './qr_shared.js';
import { selected_group } from '../../group-chats.js';
export { debounceAsync, debug, log, warn, quickReplyApi };
...
export async function init() {
    ...
    setQuickReplyApi(new QuickReplyApi(settings, manager));
    const slash = new SlashCommandHandler(getQuickReplyApi());
    slash.init();
    autoExec = new AutoExecuteHandler(settings);

    eventSource.on(event_types.APP_READY, async () => await finalizeInit());

    globalThis.quickReplyApi = getQuickReplyApi();
}
