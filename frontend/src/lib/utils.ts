...
export function parseThinkingContent(content: string): { thinkingContent: string | null; mainContent: string } {
  ...
  // 先提取 <think> 和 <model_reasoning> 标签中的内容，然后再剥离标签
  // 注意：DeepSeek V4 Pro 等模型可能直接在 content 中包含 <think>...</think> 标签
  const strippedContent = content;

  const thinkResult = extractTaggedThinking(strippedContent, 'think');
  ...
  if (!thinkingParts.length) {
    const thinkingMatch = strippedContent.match(/^([\s\S]*?)\n\n([\s\S]*)$/);
