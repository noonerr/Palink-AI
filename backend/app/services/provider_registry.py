    ...
    with _provider_cache_lock:
        ...
        try:
            ...
            if not isinstance(data, list):
                data = []
            # 全局默认思考深度：模型级默认「自动(auto)」，对话内单独设置的思考强度可覆盖单条对话。
            # 仅当模型未显式配置 reasoning_effort 时补全；用户已设为 off/low/medium/high 等则保留。
            for _p in data:
                if not isinstance(_p, dict):
                    continue
                for _m in _p.get("models", []):
                    if isinstance(_m, dict) and _m.get("id") and not _m.get("reasoning_effort"):
                        _m["reasoning_effort"] = "auto"
        except Exception:
            data = []
